...
%% ── Таблиця значень ─────────────────────────────────────────
fprintf('\n=== z2(x,y)=x*cos(x+y)  [ значення у точках МФ ] ===\n');
fprintf('%10s','');
...
function fis = make_fis(mf_type, rule_tbl, ...
    ...
    for inp=1:2
        nm=cfg{inp}{1}; lo=cfg{inp}{2}; hi=cfg{inp}{3}; pfx=cfg{inp}{4};
        s=st(inp);
        ...
        for k=1:n_in
            ...
            switch mf_type
              case 'gaussmf'
                fis=addMF(fis,nm,'gaussmf',[s*.45 c],'Name',[pfx,num2str(k)]);
              case 'trimf'
                fis=addMF(fis,nm,'trimf',[max(lo,c-s) c min(hi,c+s)],...
                          'Name',[pfx,num2str(k)]);
              case 'trapmf'
                fis=addMF(fis,nm,'trapmf',...
                    [max(lo,c-s*.95) max(lo,c-s*.35) ...
                     min(hi,c+s*.35) min(hi,c+s*.95)],...
                    'Name',[pfx,num2str(k)]);
            end
        end
    end
    ...
    for k=1:n_out
        ...
        switch mf_type
          case 'gaussmf'
            fis=addMF(fis,'z','gaussmf',[st_z*.4 c],'Name',['mf',num2str(k)]);
          case 'trimf'
...
for m=1:3
    ...
    if m==1, assignin('base','fis_z2_gauss',fis36); end
end

%% ── Графік 1: 3D еталон ─────────────────────────────────────
figure('Name','z2 — Еталон','Color','w','Position',[50 50 600 470]);
[Xg,Yg]=meshgrid(linspace(X_MIN,X_MAX,50),linspace(Y_MIN,Y_MAX,50));
surf(Xg,Yg,f(Xg,Yg),'EdgeColor','none'); colormap plasma; colorbar;
xlabel('x'); ylabel('y'); zlabel('z2'); grid on;
title('Еталонна поверхня  z_2 = x\cdotcos(x+y)','FontSize',12);

%% ── Графік 2: heatmap правил ────────────────────────────────
figure('Name','z2 — Таблиця правил','Color','w','Position',[680 50 500 420]);
imagesc(Rule_tbl); colormap(gca,parula); colorbar;
set(gca,'XTickLabel',arrayfun(@(k)['mx',num2str(k)],1:N_IN,'UniformOutput',false),...
        'YTickLabel',arrayfun(@(k)['my',num2str(k)],1:N_IN,'UniformOutput',false),'FontSize',10);
xlabel('Вхід x'); ylabel('Вхід y');
title('Таблиця правил — z_2 = x\cdotcos(x+y)');
for i=1:N_IN, for j=1:N_IN
...
%% ── Графік 3: МФ входу ──────────────────────────────────────
figure('Name','z2 — МФ','Color','w','Position',[50 560 900 300]);
...
sgtitle('МФ входу x  —  z_2(x,y) = x\cdotcos(x+y)');

%% ── Графік 4: порівняння похибок ────────────────────────────
figure('Name','z2 — Похибки','Color','w','Position',[680 520 640 400]);
xb=1:3; wb=0.35;
...
title('z_2 = x\cdotcos(x+y)  — Порівняння похибок');
grid on;
for k=1:3
...
fprintf('\n╔══════════════════════════════════════════════════════════╗\n');
fprintf('║   ВИСНОВКИ  z2(x,y) = x*cos(x+y)                       ║\n');
fprintf('╠══════════════════════════════════════════════════════════╣\n');
