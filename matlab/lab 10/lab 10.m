...
W = zeros(m, n+1);  % ваги з зміщенням
max_epochs = 100;
converged  = false;
...
for k = 1:4
    xk    = [1; patterns(:,k)];
    S     = W * xk;
    y_out = sign(S);  y_out(y_out == 0) = -1;
    [~, idx] = max(y_out);
    status = 'OK'; if idx ~= k; status = 'ПОМИЛКА'; end
...
for nl = [0.10, 0.20]
    ...
    for k = 1:4
        x_noisy = patterns(:,k);
        idx_flip = randperm(n, round(nl*n));
        x_noisy(idx_flip) = -x_noisy(idx_flip);
        xk    = [1; x_noisy];
        S     = W * xk;
        y_out = sign(S);  y_out(y_out == 0) = -1;
        [~, rec_idx] = max(y_out);
        status = 'OK'; if rec_idx ~= k; status = 'ПОМИЛКА'; end
...
for i = 1:4
    subplot(1,4,i);
    imagesc(reshape(patterns(:,i),7,5)); colormap(gray); axis off;
...
for i = 1:4
    subplot(1,4,i);
    imagesc(reshape(W(i,2:end),7,5)); colorbar; axis off;
